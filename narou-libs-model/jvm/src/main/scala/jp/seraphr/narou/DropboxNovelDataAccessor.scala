...
class DropboxNovelDataAccessor(aClient: DbxClientV2, aRootPath: String) extends NovelDataAccessor {
  ...
  private def uploadString(aPath: String, aData: String): Task[Unit] = Task
    .eval[Unit] {
      aClient
        .files()
        .uploadBuilder(aPath)
  ...
  private def downloadString(aPath: String): Task[String] = Task
    .eval {
  ...
  override def exists(): Task[Boolean] = Task
    .eval {
  ...
  override def writeNovel(aDir: String, aFile: String, aNovelStrings: Observable[String]): Task[Int] = Task.defer {
    ...
    def upload(aPath: Path): Task[Unit] = Task
      .eval[Unit] {
