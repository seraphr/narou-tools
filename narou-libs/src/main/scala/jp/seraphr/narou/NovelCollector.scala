    ...
    def collectBySettings(aRemainSettings: Vector[SearchSetting]): Iterator[Novel] = aRemainSettings match {
      ...
      case Vector(h, t @ _*) =>
        ...
        if (tRemainSize % 100 == 0) {
          ...
          logger.info(
            f"初期検索条件=${tInitSettingsSize} 残り検索条件=${tRemainSize} 進捗=${100.0 - (tRemainSize.toDouble / tInitSettingsSize * 100)}%.4f%%"
          )
        ...
        if (tHead.size < mLimit) {
          ...
          logger.info(s"検索件数(skip=${h.skip}, size=${tHead.size})が最大値未満だったため、${tRemoved
              .size} 件の検索条件を削除しました。 remain=${tNewRemain.size}。 条件: ${h.filter}")
