...
object Sandbox extends App {
  ...
  (tSettingsList zip tFavList).foreach { case (tSettings, tFav) =>
    ...
    try {
      ...
      tRankResult.novels.zipWithIndex.foreach { case (NovelAndRate(tNovel, tRate), tIndex) =>
        val tUrl = s"http://ncode.syosetu.com/${tNovel.getNcode}/"
        val tKeywords = s"[${tNovel.getKeyword}]"
        val tRateStr = "%.4f".format(tRate)
        val tLine = s"${tIndex + 1}\t${tUrl}\t${tNovel.getNumberOfChar}\t${tNovel.getAllPoint}\t${tNovel.getFavCount}\t${tRateStr}\t${tNovel.getTitle.replaceAll("[\r\n]", "")}\t${tKeywords}\n"
